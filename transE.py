...
class TransE:
    ...
    def data_initialise(self):
        ...
        for entity in self.entities:
            entity_vector = np.random.uniform(0.0 / np.sqrt(self.dimension), 8.0 / np.sqrt(self.dimension),
                                              self.dimension)
            entityVectorList[entity] = entity_vector

        for relation in self.relations:
            relation_vector = np.random.uniform(0.0 / np.sqrt(self.dimension), 8.0 / np.sqrt(self.dimension),
                                                self.dimension)
    ...
    def training_run(self, epochs=1, nbatches=100, out_file_title=''):
        ...
        with codecs.open(out_file_title + "TransE_entity_" + str(self.dimension) + "dim_batch" + str(batch_size),
            ...
            for e in self.entities.keys():
                ...
                f1.write("\n")
                try:
                    redis_client.set(e, str(list(self.entities[e])))
                except redis.exceptions.DataError:
                    print(e, str(list(self.entities[e])))

            # mapping = {e: str(list(self.entities[e])) for e in self.entities.keys()}
            # print(mapping)
            # try:
        ...
        with codecs.open(out_file_title + "TransE_relation_" + str(self.dimension) + "dim_batch" + str(batch_size),
            ...
            for r in self.relations.keys():
                ...
                f2.write(r)
                f2.write("\n")
                redis_client.set(r, str(list(self.relations[r])))
                try:
                    redis_client.set(r, str(list(self.relations[r])))
                except redis.exceptions.DataError:
                    print(r, str(list(self.relations[r])))

            # mapping = {r: str(list(self.relations[r])) for r in self.relations.keys()}
            # redis_client.mset(mapping)
    ...
    def train(self):
        ...
        self.training_run(out_file_title="umls_")
...
if __name__ == '__main__':
    ...
    # i = 0
    # for key in transE.entities.keys():
    #     if i > 100:
    #         break
    #     print(transE.entities[key])
    #     i += 1
    # i = 0
    # for key in transE.relations.keys():
    #     if i > 100:
    #         break
    #     print(transE.relations[key])
    #     i += 1
