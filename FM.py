...
def FM(dataMatrix, classLabels, k, iter):
    ...
    # 初始化参数
    w_0 = 0  # 常数项初始系数
    w_1 = np.zeros((n, 1))  # 一阶特征的初始系数
    v = normalvariate(0, 0.2) * np.ones((n, k))  # 二阶交叉特征的系数初始系数

    for it in range(iter):
        for x in range(m):  # 随机优化，每次只使用一个样本
            # 二阶项的计算
            inter_1 = dataMatrix[x] * v  # 每个样本(1*n)x(n*k),得到k维向量
            inter_2 = np.multiply(dataMatrix[x], dataMatrix[x]) * np.multiply(v, v)  # 二阶交叉项计算，得到k维向量
...
if __name__ == '__main__':
    ...
    print("开始训练")
    w_0, w_1, v = FM(np.mat(dataTrain), labelTrain, 20, 100)
